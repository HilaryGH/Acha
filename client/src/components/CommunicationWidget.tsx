...
function CommunicationWidget() {
  ...
  const handleAppChatClick = () => {
    window.location.href = appChatLink;
  };
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        <button
          ...
          className={`w-16 h-16 rounded-full bg-gradient-to-br from-green-600 to-green-700 text-white shadow-2xl hover:shadow-green-500/50 transition-all duration-300 flex items-center justify-center group ${
        ...
        {isOpen && (
          <div className="absolute bottom-20 right-0 mb-4 space-y-3 animate-fade-in-up">
          {/* Phone Option */}
          <button
            onClick={handlePhoneClick}
            className="flex items-center gap-3 bg-white text-gray-900 px-5 py-3 rounded-full shadow-xl hover:shadow-2xl hover:bg-green-50 transition-all duration-300 hover:scale-105 w-full min-w-[200px] group"
            aria-label="Call us"
          >
          ...
          >
            <div className="w-12 h-12 rounded-full bg-[#25D366] flex items-center justify-center group-hover:bg-[#20BA5A] transition-colors">
          ...
          >
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-green-600 to-blue-600 flex items-center justify-center group-hover:from-green-700 group-hover:to-blue-700 transition-colors">
              <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
            </div>

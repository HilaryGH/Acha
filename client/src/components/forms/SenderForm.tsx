  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 'success') {
        ...
        setMessage({ type: 'error', text: response.message || 'Failed to register sender' });
      }
    } catch (error: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="bg-white rounded-xl shadow-lg p-8">
        ...
        <h2 className="text-3xl font-bold mb-6 text-gray-900">Register as Sender</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? 'Submitting...' : 'Register as Sender'}
          </button>
        </form>
      </div>
    </div>
